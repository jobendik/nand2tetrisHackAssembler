...
namespace Nes
{
	namespace Core
	{
		...
		Result Apu::SetVolume(bool muteChannel[MAX_NESD_CHANNELS])
		{
			for (uint i=0; i < MAX_NESD_CHANNELS; ++i)
			{
				if (muteChannel[i])
				{
					settings.volumes[i] = 0;
				}
				else
				{
					settings.volumes[i] = 100;
				}
			}
...
//			LOGD("Apu::EndFrame: cycles=%d frameCycles=%d", cpu.GetCycles(), cpu.GetFrameCycles());
		...
		bool Apu::Square::CanOutput() const
		{
		...
		bool Apu::Triangle::CanOutput() const
		{
		...
		bool Apu::Noise::CanOutput() const
		{
		...
		Cycle Apu::Clock()
		{
//			LOGD("Apu::Clock: cycles.dmcClock=%d cycles.count=%d cycles.frameIrqClock=%d", cycles.dmcClock, cpu.GetCycles(), cycles.frameIrqClock);
...
//			LOGD("Apu::Clock:    ret MIN=%d", NST_MIN(cycles.dmcClock,cycles.frameIrqClock));
		...
		NST_NO_INLINE Apu::Channel::Sample Apu::GetSample()
		{
			...
			dword ch0 = square[0].GetSample();
			dword ch1 = square[1].GetSample();
			dword ch2 = triangle.GetSample();
			dword ch3 = noise.GetSample();
			dword ch4 = dmc.GetSample();
			dword ch5 = (extChannel ? extChannel->GetSample() : 0);
			
			idword mix = Clamp<Channel::OUTPUT_MIN,Channel::OUTPUT_MAX>
			(
				dcBlocker.Apply
				(
					(0 != (dac[0] = ch0 + ch1) ? NLN_SQ_0 / (NLN_SQ_1 / dac[0] + NLN_SQ_2) : 0) +
					(0 != (dac[1] = ch2 + ch3 + ch4) ? NLN_TND_0 / (NLN_TND_1 / dac[1] + NLN_TND_2) : 0)
				) + ch5
			);
			
////			LOGD("nesd_isReceiveChannelsData=%s", STRBOOL(nesd_isReceiveChannelsData));
//			if (nesd_isReceiveChannelsData)
			{
				// TODO: remove nesd_isReceiveChannelsData, this will be decided by the view's visibility
				nesd_receive_channels_data(ch0, ch1, ch2, ch3, ch4, ch5, mix);
			}
		...
		//
		void Apu::SaveNesDebuggerState(CByteBuffer *byteBuffer)
		{
			byteBuffer->PutU32(ctrl);
			byteBuffer->PutU32(cycles.fixed);
			byteBuffer->PutU64(cycles.rate);
			byteBuffer->PutU64(cycles.rateCounter);
			byteBuffer->PutU64(cycles.frameCounter);
			byteBuffer->PutU64(cycles.extCounter);
			byteBuffer->PutU16(cycles.frameDivider);
			byteBuffer->PutU16(cycles.frameIrqRepeat);
			byteBuffer->PutU64(cycles.frameIrqClock);
			byteBuffer->PutU64(cycles.dmcClock);
			byteBuffer->PutU32(synchronizer.sync);
			byteBuffer->PutU32(synchronizer.duty);
			byteBuffer->PutU32(synchronizer.streamed);
			byteBuffer->PutU32(synchronizer.rate);
		}
	
		void Apu::LoadNesDebuggerState(CByteBuffer *byteBuffer)
		{
			ctrl = byteBuffer->GetU32();
			cycles.fixed = byteBuffer->GetU32();
			cycles.rate = byteBuffer->GetU64();
			cycles.rateCounter = byteBuffer->GetU64();
			cycles.frameCounter = byteBuffer->GetU64();
			cycles.extCounter = byteBuffer->GetU64();
			cycles.frameDivider = byteBuffer->GetU16();
			cycles.frameIrqRepeat = byteBuffer->GetU16();
			cycles.frameIrqClock = byteBuffer->GetU64();
			cycles.dmcClock = byteBuffer->GetU64();
			synchronizer.sync = byteBuffer->GetU32();
			synchronizer.duty = byteBuffer->GetU32();
			synchronizer.streamed = byteBuffer->GetU32();
			synchronizer.rate = byteBuffer->GetU32();
		}
	}
}
