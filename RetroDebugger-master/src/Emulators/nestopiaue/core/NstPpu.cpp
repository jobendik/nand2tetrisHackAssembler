...
//			LOGD("Ppu::BeginFrame: output.pixels=%x", output.pixels);
		...
		NST_FORCE_INLINE void Ppu::RenderPixel()
		{
			...
			// TODO: CONTRA CRASHED HERE DUE TO NOT AVAILABLE SAVE
		...
		void Ppu::SaveNesDebuggerState(CByteBuffer *byteBuffer)
		{
			// save full state, needed for NES Debugger
			byteBuffer->PutU32(regs.ctrl[0]);
			byteBuffer->PutU32(regs.ctrl[1]);
			byteBuffer->PutU32(regs.status);
			byteBuffer->PutU32(regs.frame);
			byteBuffer->PutU32(regs.oam);
			
			byteBuffer->PutU32(scroll.address);
			byteBuffer->PutU32(scroll.toggle);
			byteBuffer->PutU32(scroll.latch);
			byteBuffer->PutU32(scroll.xFine);
			
			byteBuffer->PutU8(tiles.pattern[0]);
			byteBuffer->PutU8(tiles.pattern[1]);
			byteBuffer->PutU8(tiles.attribute);
			byteBuffer->PutU8(tiles.index);
			byteBuffer->PutBytes(tiles.pixels, 16);
			byteBuffer->PutU32(tiles.mask);
			byteBuffer->PutU8(tiles.show[0]);
			byteBuffer->PutU8(tiles.show[1]);
			
			byteBuffer->PutBytes(palette.ram, palette.SIZE);
			
			u32 pixelNum = output.target - output.pixels;
			byteBuffer->PutU32(pixelNum);
			byteBuffer->PutU32(output.burstPhase);
			for (int i = 0; i < Palette::SIZE; i++)
			{
				byteBuffer->PutU16(output.palette[i]);
			}
			byteBuffer->PutU32(output.bgColor);
			
			// oam
			byteBuffer->PutBytes(oam.ram, 0x0100);
			byteBuffer->PutBytes(oam.buffer, Oam::MAX_LINE_SPRITES * 4);
			
			// oam
			byteBuffer->PutU8(oam.visible->x);
			byteBuffer->PutU8(oam.visible->behind);
			byteBuffer->PutU8(oam.visible->zero);
			byteBuffer->PutU8(oam.visible->palette);
			byteBuffer->PutBytes(oam.visible->pixels, 8);
			byteBuffer->PutU32(oam.latch);
			byteBuffer->PutU32(oam.index);
			byteBuffer->PutU32(oam.address);
			byteBuffer->PutU32(oam.height);
			byteBuffer->PutU32(oam.mask);
			byteBuffer->PutBytes(oam.show, 2);
			byteBuffer->PutBool(oam.spriteZeroInLine);
			byteBuffer->PutBool(oam.spriteLimit);
			
			for (int i = 0; i < oam.MAX_LINE_SPRITES; i++)
			{
				byteBuffer->PutU8(oam.output[i].x);
				byteBuffer->PutU8(oam.output[i].behind);
				byteBuffer->PutU8(oam.output[i].zero);
				byteBuffer->PutU8(oam.output[i].palette);
				byteBuffer->PutBytes(oam.output[i].pixels, 8);
			}
			
			byteBuffer->PutBytes(nameTable.ram, nameTable.SIZE);
			
			byteBuffer->PutU32(io.address);
			byteBuffer->PutU32(io.pattern);
			byteBuffer->PutU32(io.latch);
			byteBuffer->PutU32(io.buffer);

			byteBuffer->PutU32(cycles.count);
			byteBuffer->PutU32(cycles.hClock);
			byteBuffer->PutU32(cycles.vClock);
			byteBuffer->PutU32(cycles.one);
			byteBuffer->PutU32(cycles.reset);
		...
		{
			regs.ctrl[0] = byteBuffer->GetU32();
			regs.ctrl[1] = byteBuffer->GetU32();
			regs.status = byteBuffer->GetU32();
			regs.frame = byteBuffer->GetU32();
			regs.oam = byteBuffer->GetU32();
			
			scroll.address = byteBuffer->GetU32();
			scroll.toggle = byteBuffer->GetU32();
			scroll.latch = byteBuffer->GetU32();
			scroll.xFine = byteBuffer->GetU32();
			
			tiles.pattern[0] = byteBuffer->GetU8();
			tiles.pattern[1] = byteBuffer->GetU8();
			tiles.attribute = byteBuffer->GetU8();
			tiles.index = byteBuffer->GetU8();
			byteBuffer->GetBytes(tiles.pixels, 16);
			tiles.mask = byteBuffer->GetU32();
			tiles.show[0] = byteBuffer->GetU8();
			tiles.show[1] = byteBuffer->GetU8();
			
			byteBuffer->GetBytes(palette.ram, palette.SIZE);
			
			u32 pixelNum = byteBuffer->GetU32();
			output.target = output.pixels + pixelNum;
			output.burstPhase = byteBuffer->GetU32();
			for (int i = 0; i < Palette::SIZE; i++)
			{
				output.palette[i] = byteBuffer->GetU16();
			}
			output.bgColor = byteBuffer->GetU32();
			
			// oam
			byteBuffer->GetBytes(oam.ram, 0x0100);
			byteBuffer->GetBytes(oam.buffer, Oam::MAX_LINE_SPRITES * 4);
			
			// oam
			oam.visible->x = byteBuffer->GetU8();
			oam.visible->behind = byteBuffer->GetU8();
			oam.visible->zero = byteBuffer->GetU8();
			oam.visible->palette = byteBuffer->GetU8();
			byteBuffer->GetBytes(oam.visible->pixels, 8);
			oam.latch = byteBuffer->GetU32();
			oam.index = byteBuffer->GetU32();
			oam.address = byteBuffer->GetU32();
			oam.height = byteBuffer->GetU32();
			oam.mask = byteBuffer->GetU32();
			byteBuffer->GetBytes(oam.show, 2);
			oam.spriteZeroInLine = byteBuffer->GetBool();
			oam.spriteLimit = byteBuffer->GetBool();
			
			for (int i = 0; i < oam.MAX_LINE_SPRITES; i++)
			{
				oam.output[i].x = byteBuffer->GetU8();
				oam.output[i].behind = byteBuffer->GetU8();
				oam.output[i].zero = byteBuffer->GetU8();
				oam.output[i].palette = byteBuffer->GetU8();
				byteBuffer->GetBytes(oam.output[i].pixels, 8);
			}
			
			byteBuffer->GetBytes(nameTable.ram, nameTable.SIZE);
			
			io.address = byteBuffer->GetU32();
			io.pattern = byteBuffer->GetU32();
			io.latch = byteBuffer->GetU32();
			io.buffer = byteBuffer->GetU32();

			cycles.count = byteBuffer->GetU32();
			cycles.hClock = byteBuffer->GetU32();
			cycles.vClock = byteBuffer->GetU32();
			cycles.one = byteBuffer->GetU32();
			cycles.reset = byteBuffer->GetU32();
