...
namespace Nes
{
	namespace Core
	{
		...
		{
			nesdMainCpuCycle = 0;
			nesdMainCpuDebugCycle = 0;
			nesdMainCpuPreviousInstructionCycle = 0;
		...
		{
//			LOGD("Peek8: %04x", address);
			NST_ASSERT( address < FULL_SIZE );
			
			nesd_mark_cell_read(address);
			
			return ports[address].Peek( address );
		}

		inline uint Cpu::IoMap::Peek8_NoMarking(const uint address) const
		{
			//			LOGD("Peek8: %04x", address);
			NST_ASSERT( address < FULL_SIZE );
			return ports[address].Peek( address );
		...
		{
//			LOGD("Peek16: %04x", address);
			NST_ASSERT( address < FULL_SIZE-1 );
			
			nesd_mark_cell_read(address);
			nesd_mark_cell_read(address + 1);
		...
		inline void Cpu::IoMap::Poke8(const uint address,const uint data) const
		{
			...
			nesd_mark_cell_write(address, data);
			
			ports[address].Poke( address, data );
		}

		void Cpu::IoMap::Poke8_NoMarking(const uint address,const uint data) const
		{
			NST_ASSERT( address < FULL_SIZE );
			ports[address].Poke( address, data );
		}
		...
		{
//			TODO: bug, powerstate is not initalised variable
//			Conditional jump or move depends on uninitialised value(s)
//			==249397==    at 0x4E5AAD: Nes::Core::Cpu::Ram::Reset(Nes::Core::CpuModel) (NstCpu.cpp:816)
		...
		uint Cpu::IndY_R()
		{
			...
			nesd_mark_cell_read(data);
			const uint indexed = ram.mem[data] + y;

			nesd_mark_cell_read((data + 1) & 0xFF);
		...
		inline uint Cpu::IndY_RW(uint& data)
		{
			...
			nesd_mark_cell_read((address + 1) & 0xFF);
		...
		inline void Cpu::Push8(const uint data)
		{
			...
			nesd_mark_cell_write(0x100+p, data);
		...
		NST_FORCE_INLINE void Cpu::Push16(const uint data)
		{
			...
			nesd_mark_cell_write(0x100+p1, data & 0xFF);
			ram.mem[0x100+p1] = data & 0xFF;
			
			nesd_mark_cell_write(0x100+p0, data >> 8);
		...
		inline uint Cpu::Pull16()
		{
			...
			nesd_mark_cell_read(0x100+p0);
			nesd_mark_cell_read(0x100+p1);
...
//			LOGD("Cpu::Clock: clock=%d cycles.count=%d cycles.frame=%d nmiClock=%d irqClock=%d", clock, cycles.count, cycles.frame, interrupt.nmiClock, interrupt.irqClock);
...
//				LOGD("Cpu::Clock: DoISR NMI");
		...
		{
//			LOGD("Cpu::ExecuteOp: 1 cycle=%d", apu.Clock());
			cycles.offset = cycles.count;
			
			uint opcodePC = this->pc;
			const uint opcodeData = map.Peek8( pc );

			nesd_update_cpu_pc_by_emulator(opcodePC);
			nesd_mark_cell_execute(opcodePC, opcodeData);

			if (nesd_is_debug_on())
			{
				if (nesd_debug_mode != DEBUGGER_MODE_RUN_ONE_INSTRUCTION
					&& nesd_debug_mode != DEBUGGER_MODE_RUN_ONE_CYCLE)
				{
					nesd_check_pc_breakpoint(this->pc);
				}
				
				nesd_debug_pause_check(1);
			}
			
			nesdMainCpuPreviousInstructionCycle = nesdMainCpuCycle;
			dword prevCycles = cycles.count;
			...
			if (nesd_is_debug_on())
			{
				nesd_debug_pause_check(0);
			}
			
//			LOGD("Cpu::ExecuteOp: 2 cycle=%d", cycles.count);
			
			// NOTE: the nesdMainCpuCycle which is used by snapshots recorder is actually instruction counter
			dword diff = cycles.count - prevCycles;
			nesdMainCpuCycle += 1;
			nesdMainCpuDebugCycle += diff;
			...
			if (nesd_is_debug_on())
			{
				...
				if (nesd_debug_mode == DEBUGGER_MODE_RUN_ONE_INSTRUCTION)
				{
					nesd_debug_mode = DEBUGGER_MODE_PAUSED;
				}
			}
			
			// note, code will be paused on next iteration
		}
		...
		uint Cpu::Peek_NoMarking(const uint address) const
		{
			return map.Peek8_NoMarking( address );
		}
		...
		void Cpu::Poke_NoMarking(const uint address,const uint data) const
		{
			return map.Poke8_NoMarking( address, data );
		}
		...
		void Cpu::SaveNesDebuggerState(CByteBuffer *byteBuffer)
		{
//			LOGD("**** Cpu::SaveNesDebuggerState: store frame=%d instr=%d cycle=%d round=%d", nesdFrame, nesdMainCpuCycle, cycles.count, cycles.round);
			byteBuffer->PutU64(nesdMainCpuDebugCycle);
			byteBuffer->PutU64(nesdMainCpuCycle);
			byteBuffer->PutU64(nesdMainCpuPreviousInstructionCycle);
			
			byteBuffer->PutU32(pc);
			byteBuffer->PutU64(cycles.count);
			for (int i = 0; i < 8; i++)
			{
				byteBuffer->PutU8(cycles.clock[i]);
			}
			byteBuffer->PutU64(cycles.offset);
			byteBuffer->PutU64(cycles.round);
			byteBuffer->PutU64(cycles.frame);
			byteBuffer->PutU8(a);
			byteBuffer->PutU8(x);
			byteBuffer->PutU8(y);
			byteBuffer->PutU8(sp);
			byteBuffer->PutU8(flags.Pack());
			byteBuffer->PutU64(interrupt.nmiClock);
			byteBuffer->PutU64(interrupt.irqClock);
			byteBuffer->PutU64(interrupt.low);
			byteBuffer->PutU32(opcode);
			byteBuffer->PutU16(jammed);
			byteBuffer->PutU16(model);
			byteBuffer->PutU64(ticks);
			byteBuffer->PutU32(logged);
		}
		
		void Cpu::LoadNesDebuggerState(CByteBuffer *byteBuffer)
		{
			nesdMainCpuDebugCycle = byteBuffer->GetU64();
			nesdMainCpuCycle = byteBuffer->GetU64();
			nesdMainCpuPreviousInstructionCycle = byteBuffer->GetU64();
			
			pc = byteBuffer->GetU32();
			cycles.count = byteBuffer->GetU64();
			for (int i = 0; i < 8; i++)
			{
				cycles.clock[i] = byteBuffer->GetU8();
			}
			cycles.offset = byteBuffer->GetU64();
			cycles.round = byteBuffer->GetU64();
			cycles.frame = byteBuffer->GetU64();
			a = byteBuffer->GetU8();
			x = byteBuffer->GetU8();
			y = byteBuffer->GetU8();
			sp = byteBuffer->GetU8();
			flags.Unpack(byteBuffer->GetU8());
			interrupt.nmiClock = byteBuffer->GetU64();
			interrupt.irqClock = byteBuffer->GetU64();
			interrupt.low = byteBuffer->GetU64();
			opcode = byteBuffer->GetU32();
			jammed = byteBuffer->GetU16();
			model = byteBuffer->GetU16();
			ticks = byteBuffer->GetU64();
			logged = byteBuffer->GetU32();
			
			LOGD("LoadNesDebuggerState: load frame=%d instr=%d cycle=%d round=%d", nesdFrame, nesdMainCpuCycle, cycles.count, cycles.round);
		}
	}
}
