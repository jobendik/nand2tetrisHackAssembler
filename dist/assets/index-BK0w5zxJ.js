...
(DL)
    ...
    @DL
    0;JMP
`}],D=new Pt;let L=[],Tt=[],b=!1,I=1,J=new Set,S=0,m=null;const Ot=[{label:"1/s",cycles:1},{label:"10/s",cycles:10},{label:"100/s",cycles:100},{label:"1K/s",cycles:1e3},{label:"10K/s",cycles:1e4},{label:"MAX",cycles:1e5}];performance.now();let nt="registers";const h=new Float32Array(32768),f=new Float32Array(32768),Y=.04,It=document.getElementById("app");It.innerHTML=`
<!-- Top Bar -->
<div class="topbar">
  <div class="topbar-left">
    ...
    <a class="btn btn-mode-switch" href="/vm.html" title="Switch to VM Visualizer">← VM Mode</a>
  </div>
...
`;const c=t=>document.getElementById(t),A=c("code-editor"),it=c("gutter"),k=c("disasm-body"),$=c("screen-canvas"),_=c("memmap-canvas"),T=c("explain-body"),$t=c("reg-body"),j=c("instruction-body"),F=c("alu-body"),_t=c("ram-body"),rt=$.getContext("2d"),ot=_.getContext("2d"),V=rt.createImageData(512,256),z=ot.createImageData(256,128),B=c("sample-select");let X="";for(let t=0;t<y.length;t++){const e=y[t];if(e.category!==X){const s=document.createElement("optgroup");s.label=e.category,X=e.category;for(let a=t;a<y.length&&y[a].category===e.category;a++){const n=document.createElement("option");n.value=String(a),n.textContent=y[a].title,s.appendChild(n)}B.appendChild(s)}}B.addEventListener("change",()=>{const t=parseInt(B.value);!isNaN(t)&&y[t]&&(A.value=y[t].code,G())});const q=c("speed-group");Ot.forEach((t,e)=>{const s=document.createElement("button");s.className="btn speed-btn"+(e===0?" active":""),s.textContent=t.label,s.addEventListener("click",()=>{I=t.cycles,q.querySelectorAll(".speed-btn").forEach((a,n)=>{a.classList.toggle("active",n===e)})}),q.appendChild(s)});A.value=y[0].code;function G(){const t=A.value.split(`
`);let e="";for(let s=0;s<t.length;s++)e+=`<div class="gutter-line" data-line="${s}">${s+1}</div>`;it.innerHTML=e}A.addEventListener("input",G);A.addEventListener("scroll",()=>{it.scrollTop=A.scrollTop});A.addEventListener("keyup",()=>{const t=A.selectionStart,e=A.value.substring(0,t).split(`
`).length;c("line-info").textContent=`Ln ${e}`});G();c("btn-assemble").addEventListener("click",lt);function lt(){try{Tt=A.value.split(`
`);const t=xt(A.value);L=t.sourceMap,D.reset(),D.loadROM(t.rom),J.clear(),S=0,b=!1,m=null,h.fill(0),f.fill(0),kt(),T.innerHTML='<div class="explain-placeholder">Program assembled! Click <strong>STEP</strong> to begin.</div>',c("rom-count").textContent=`${L.length} instructions`}catch(t){T.innerHTML=`<div class="explain-error">❌ Assembly Error: ${H(t.message)}</div>`}}function kt(){k.innerHTML="";const t=document.createDocumentFragment();for(let e=0;e<L.length;e++){const s=L[e],a=document.createElement("div");a.className="dline",a.dataset.addr=String(e);const r=parseInt(s.bin,2).toString(16).toUpperCase().padStart(4,"0"),o=s.bin;a.innerHTML=`<span class="bp-dot" title="Click to toggle breakpoint">●</span><span class="d-addr">${e.toString(16).toUpperCase().padStart(4,"0")}</span><span class="d-hex">${r}</span><span class="d-bin">${o.substring(0,3)} <em>${o.substring(3,10)}</em> ${o.substring(10,13)} ${o.substring(13)}</span><span class="d-src">${H(s.src)}</span>`,a.addEventListener("click",()=>Ft(a,e)),t.appendChild(a)}k.appendChild(t)}function H(t){return t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function Ft(t,e){J.has(e)?(J.delete(e),t.classList.remove("breakpoint")):(J.add(e),t.classList.add("breakpoint"))}function Bt(){const t=k.children;for(let e=0;e<t.length;e++){const s=t[e];if(parseInt(s.dataset.addr)===D.PC){s.classList.add("current");const n=k,r=s.offsetTop,o=r+s.offsetHeight,i=n.scrollTop,M=i+n.clientHeight;(r<i||o>M)&&(n.scrollTop=r-n.clientHeight/3)}else s.classList.remove("current")}}c("btn-step").addEventListener("click",()=>{b=!1,Dt()});c("btn-run").addEventListener("click",()=>{b=!0});c("btn-pause").addEventListener("click",()=>{b=!1});c("btn-reset").addEventListener("click",()=>{b=!1,D.reset(),S=0,m=null,h.fill(0),f.fill(0),T.innerHTML='<div class="explain-placeholder">CPU reset. Click <strong>STEP</strong> to begin.</div>'});c("chk-micro").addEventListener("change",t=>{t.target.checked});function Dt(){var e;D.readAddrs.clear(),D.writeAddrs.clear(),m=D.stepDetailed(),S++;const t=((e=L[m.prevPC])==null?void 0:e.src)??"?";T.innerHTML=at(m,t),Gt(m.prevPC)}function Gt(t){var o;const e=L[t];if(!e)return;const s=e.lineIdx,a=A.value.split(`
`);let n=0;for(let i=0;i<s&&i<a.length;i++)n+=a[i].length+1;const r=n+(((o=a[s])==null?void 0:o.length)??0);A.focus(),A.setSelectionRange(n,r),A.blur()}const Z={Enter:128,Backspace:129,ArrowLeft:130,ArrowUp:131,ArrowRight:132,ArrowDown:133,Home:134,End:135,PageUp:136,PageDown:137,Insert:138,Delete:139,Escape:140,F1:141,F2:142,F3:143,F4:144,F5:145,F6:146,F7:147,F8:148,F9:149,F10:150,F11:151,F12:152};document.addEventListener("keydown",t=>{if(document.activeElement===A)return;if(t.key===" "){t.preventDefault(),Dt();return}if(t.key==="r"&&!t.ctrlKey){b=!0;return}if(t.key==="p"){b=!1;return}let e=0;Z[t.key]!==void 0?e=Z[t.key]:t.key.length===1&&(e=t.key.charCodeAt(0)),e&&(D.ram[24576]=e)});document.addEventListener("keyup",t=>{document.activeElement!==A&&(D.ram[24576]=0)});c("ram-tabs").addEventListener("click",t=>{const e=t.target;e.classList.contains("tab")&&(nt=e.dataset.tab,c("ram-tabs").querySelectorAll(".tab").forEach(s=>s.classList.remove("active")),e.classList.add("active"))});let O=!1;c("alu-toggle").addEventListener("click",()=>{O=!O,F.style.display=O?"none":"",c("alu-toggle").textContent=O?"▶":"▼"});function C(t){return((t&65535)>>>0).toString(16).toUpperCase().padStart(4,"0")}function N(t){return t=t&65535,t>=32768?t-65536:t}let tt=0,et=0,st=0;function Ht(){const t=D.A&65535,e=N(D.D),s=D.PC,a=D.A&32767,n=N(D.ram[a]),r=t!==tt?" flash":"",o=e!==N(et)?" flash":"",i=s!==st?" flash":"";$t.innerHTML=`
    <div class="reg-row">
      <div class="reg-box${r}">
    ...
    <div class="m-ribbon">
      ...
      <span class="m-detail">RAM[${a}] = <strong>${n}</strong> (0x${C(n)})</span>
        ...
        </div>`}}function Wt(){if(!m||m.isA||!m.alu){F.innerHTML='<div class="alu-empty">No C-instruction yet — step through one to see the ALU.</div>';return}const t=m.alu,e=(s,a)=>`<div class="alu-bit"><span class="alu-bit-name">${s}</span><span class="alu-bit-val ${a?"on":"off"}">${a}</span></div>`;F.innerHTML=`
    ...
    </div>`}function Kt(){let t="";const e=(s,a,n=!1)=>{const r=N(D.ram[s&32767]),o=n?' class="ram-highlight"':"",i=a??`RAM[${s}]`,M=f[s]>.3?" ram-written":"",l=h[s]>.3?" ram-read":"";return`<div class="ram-row${M}${l}"${o}><span class="ram-addr">${i}</span><span class="ram-val">${r}</span><span class="ram-hex">0x${C(r)}</span></div>`};switch(nt){case"registers":t+='<div class="ram-section-label">Special Pointers</div>',t+=e(0,"SP (R0)"),t+=e(1,"LCL (R1)"),t+=e(2,"ARG (R2)"),t+=e(3,"THIS (R3)"),t+=e(4,"THAT (R4)"),t+='<div class="ram-section-label">Temp Registers</div>';for(let s=5;s<=12;s++)t+=e(s,`R${s}`);t+='<div class="ram-section-label">R13–R15 (static)</div>';for(let s=13;s<=15;s++)t+=e(s,`R${s}`);break;case"aroundA":{const s=D.A&32767,a=Math.max(0,s-8),n=Math.min(32767,s+8);for(let r=a;r<=n;r++)t+=e(r,`RAM[${r}]`,r===s);break}case"stack":{const s=D.ram[0];t+=`<div class="ram-section-label">SP = ${s}</div>`;const a=Math.max(256,s-4);for(let n=a;n<a+16&&n<32768;n++)t+=e(n,`RAM[${n}]`,n===s);break}case"screen":{t+='<div class="ram-section-label">Screen (first 16 words)</div>';for(let s=0;s<16;s++){const a=16384+s;t+=e(a,`SCR[${s}]`)}t+='<div class="ram-section-label">KBD</div>',t+=e(24576,"KBD");break}}_t.innerHTML=t}function Qt(){const t=V.data;for(let e=0;e<256;e++)for(let s=0;s<32;s++){const a=D.ram[At+e*32+s];for(let n=0;n<16;n++){const r=s*16+n,o=(e*512+r)*4,i=a>>n&1;t[o]=i?160:15,t[o+1]=i?230:18,t[o+2]=i?180:25,t[o+3]=255}}rt.putImageData(V,0,0)}function Yt(){const t=z.data;for(let e=0;e<32768;e++)h[e]>0&&(h[e]-=Y,h[e]<0&&(h[e]=0)),f[e]>0&&(f[e]-=Y,f[e]<0&&(f[e]=0));for(const e of D.readAddrs)h[e]=1;for(const e of D.writeAddrs)f[e]=1;for(let e=0;e<32768;e++){const s=e%256,n=((e/256|0)*256+s)*4,r=D.ram[e],o=f[e],i=h[e];let M=r!==0?25:8,l=r!==0?28:10,p=r!==0?35:14;o>.01&&(M+=o*220,l+=o*40),i>.01&&(l+=i*180,p+=i*120),t[n]=Math.min(255,M)|0,t[n+1]=Math.min(255,l)|0,t[n+2]=Math.min(255,p)|0,t[n+3]=255}ot.putImageData(z,0,0)}function jt(){c("stat-cycles").textContent=`${S.toLocaleString()} cycles`;const t=c("stat-state");t.textContent=b?"RUNNING":"STOPPED",t.className="stat-pill "+(b?"running":"stopped")}function Mt(t){var e;if(requestAnimationFrame(Mt),b){if(D.readAddrs.clear(),D.writeAddrs.clear(),I<=100){for(let s=0;s<I;s++)if(m=D.stepDetailed(),S++,J.has(D.PC)){b=!1;break}if(m){const s=((e=L[m.prevPC])==null?void 0:e.src)??"?";T.innerHTML=at(m,s)}}else for(let s=0;s<I;s++)if(D.step(),S++,J.has(D.PC)){b=!1;break}}Ht(),Ut(),Wt(),Kt(),Yt(),Qt(),Bt(),jt()}function ct(){const t=$.parentElement,e=t.clientWidth-4,s=t.clientHeight-4,a=Math.max(.5,Math.min(e/512,s/256));$.style.width=Math.floor(512*a)+"px",$.style.height=Math.floor(256*a)+"px";const n=_.parentElement,r=n.clientWidth-4,o=n.clientHeight-4,i=Math.max(1,Math.min(Math.floor(r/256),Math.floor(o/128)));_.style.width=256*i+"px",_.style.height=128*i+"px"}window.addEventListener("resize",ct);setTimeout(ct,100);lt();requestAnimationFrame(Mt);
